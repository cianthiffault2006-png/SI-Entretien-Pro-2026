...
export default async function PayrollPage() {
  ...
  // Periods sorted newest first (same as leaderboard)
  const { data: periods } = await supabase
    .from('pay_periods')
    .select('*')
    .gte('end_date', '2026-01-01') // only 2026+
  ...
  const periodPaychecks = allPeriodsArr.map(period => {
    ...
    const repBreakdown = (reps || []).map(rep => {
      ...
      // Cumulative D2D before this period for tier calculation
  ...
  // Find current period index (default selection)
