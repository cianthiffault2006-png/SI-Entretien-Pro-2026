...
export default function Nav({ profile }: { profile: Profile }) {
  ...
  return (
    <>
      ...
      <aside className="sm-sidebar" style={{
        display: 'none', flexDirection: 'column',
        width: 200, minWidth: 200, flexShrink: 0,
        minHeight: '100vh', background: '#0A1628', borderRight: '1px solid #1E3A5F',
      }}>
        ...
        <nav style={{ flex: 1, padding: 8, overflowY: 'auto' }}>
          {items.filter(i => i.href !== '/settings').map(item => (
            <Link key={item.href} href={item.href} style={{
              ...
              marginBottom: 2, textDecoration: 'none', fontSize: 12, fontWeight: 500,
      ...
      {/* ══ MOBILE TOP BAR (<640px) ══
          MINIMAL: just a small SI badge + hamburger. No text, no name, no wasted space. */}
      <header className="mobile-topbar" style={{
        display: 'none', position: 'sticky', top: 0, zIndex: 40,
        alignItems: 'center', justifyContent: 'space-between',
        padding: '0 10px', height: 44,
        background: '#0A1628', borderBottom: '1px solid #1E3A5F', flexShrink: 0,
      }}>
        {/* Just the SI badge - tapping goes home */}
        <Link href="/dashboard" style={{ textDecoration: 'none' }}>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: '#1B9EF3', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 900, fontSize: 13 }}>SI</div>
        </Link>

        {/* Right: avatar initial + hamburger */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 30, height: 30, borderRadius: '50%', background: color + '22', border: `1.5px solid ${color}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color }}>
            {initials}
          </div>
          <button onClick={() => setOpen(true)} aria-label="Menu"
                  style={{ width: 34, height: 34, borderRadius: 8, background: '#132D45', border: '1px solid #1E3A5F', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, cursor: 'pointer', padding: 0 }}>
        ...
        <div className="mobile-drawer-root">
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 260, maxWidth: '82vw', background: '#0A1628', borderLeft: '1px solid #1E3A5F', zIndex: 51, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: '1px solid #1E3A5F', flexShrink: 0 }}>
              ...
              <button onClick={() => setOpen(false)} style={{ background: 'none', border: 'none', color: '#6B8AA8', fontSize: 26, cursor: 'pointer', lineHeight: 1, padding: '0 4px' }}>×</button>
            </div>
            {/* Nav links */}
      ...
      <nav className="mobile-tabs" style={{
        ...
        alignItems: 'stretch', justifyContent: 'space-around', height: 54,
      }}>
        {tabItems.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href} style={{
              ...
              gap: 2, flex: 1, textDecoration: 'none', color: on ? '#1B9EF3' : '#5A8AA8',
              ...
              <span style={{ fontSize: 9, fontWeight: on ? 700 : 400, lineHeight: 1, letterSpacing: 0.2 }}>{item.label}</span>
            </Link>
          );
        })}
      </nav>

      {/* Responsive toggle — pure CSS, no Tailwind dependency */}
      <style>{`
