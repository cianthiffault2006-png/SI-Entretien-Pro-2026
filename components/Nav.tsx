...
// Admin sees everything. Reps/cleaners see simplified view.
...
const TABS: Record<string, string[]> = {
  ...
  cleaner: ['/dashboard', '/disponibilite'],
};
...
export default function Nav({ profile }: { profile: Profile }) {
  ...
  return (
    <>
      ...
      {/* ── MOBILE TOP BAR ─────────────────────────────────────── */}
      <header className="mobile-topbar" style={{ display: 'none', position: 'sticky', top: 0, zIndex: 40, alignItems: 'center', justifyContent: 'space-between', padding: '0 12px', height: 48, background: '#0A1628', borderBottom: '1px solid #1E3A5F', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={() => setOpen(true)} style={{ width: 36, height: 36, borderRadius: 8, background: '#132D45', border: '1px solid #1E3A5F', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, cursor: 'pointer' }}>
        ...
        <div className="mobile-drawer-root">
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 260, maxWidth: '80vw', background: '#0A1628', borderLeft: '1px solid #1E3A5F', zIndex: 51, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: '1px solid #1E3A5F' }}>
              ...
              <button onClick={() => setOpen(false)} style={{ background: 'none', border: 'none', color: '#6B8AA8', fontSize: 24, cursor: 'pointer', lineHeight: 1 }}>×</button>
            </div>
            <nav style={{ flex: 1, padding: '10px 10px' }}>
              {items.map(item => (
                <Link key={item.href} href={item.href} onClick={() => setOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 12px', borderRadius: 10, marginBottom: 3, textDecoration: 'none', fontSize: 14, fontWeight: 500, background: active(item.href) ? '#1B9EF322' : 'transparent', color: active(item.href) ? '#1B9EF3' : '#C2D4E8', borderLeft: `3px solid ${active(item.href) ? '#1B9EF3' : 'transparent'}` }}>
      ...
      {/* ── MOBILE BOTTOM TABS ──────────────────────────────────── */}
      <nav className="mobile-tabs" style={{ display: 'none', position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 40, background: '#0A1628', borderTop: '1px solid #1E3A5F', alignItems: 'center', justifyContent: 'space-around', height: 56 }}>
        {tabItems.map(item => {
          const on = active(item.href);
          return (
            <Link key={item.href} href={item.href} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, flex: 1, height: '100%', textDecoration: 'none', color: on ? '#1B9EF3' : '#6B8AA8', position: 'relative' }}>
